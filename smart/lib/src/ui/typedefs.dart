...
/// A callback function that is invoked when a new page request is made.
///
/// The `PagedRequestListener` typedef defines a function signature for
/// callbacks that are invoked when a request is made to load a specific page
/// of data. The function receives a `PageKeyType` as a parameter, which
/// typically represents the key or identifier of the page being requested.
typedef PagedRequestListener<PageKeyType> = void Function(PageKeyType pageKey);

/// A callback function that is invoked when the paging status changes.
///
/// The `PagedStatusListener` typedef defines a function signature for
/// callbacks that are invoked when the paging status changes.
/// The `PagingStatus` enum represents the current state of the pagination
/// operation (e.g., loading, completed, error).
typedef PagedStatusListener = void Function(PagedStatus status);
...
/// A function that builds a scrollable widget with paginated data.
///
/// Takes the total item count, pagination status, and an [IndexedWidgetBuilder]
/// to create widgets like [ListView] or [GridView].
///
/// - [itemCount] The total number of items.
/// - [status] The current pagination status.
/// - [child] The builder for individual items.
/// - [showExtra] A boolean indicating whether to show an extra widget.
///
/// **Example:**
/// ```dart
/// PagedChildBuilder<int, Item> childBuilder = (itemCount, status, itemBuilder) {
...
/// };
/// ```
typedef PagedChildBuilder<PageKeyType, ItemType> = Widget Function(
  int itemCount,
  PagedStatus status,
  Boolean showExtra,
  IndexedWidgetBuilder child,
);
...
///
/// ```dart
/// PagedItemSeparatorStrategy separatorStrategy = (index) {
///   // Insert a separator after every 5th item
///   return index > 0 && index % 5 == 0;
/// };
/// ```
typedef PagedItemSeparatorStrategy = Boolean Function(int index);

...
class TextAutoSizing extends StatefulWidget {
  ...
  /// Synchronizes the size of multiple [TextAutoSizing]s.
  ///
  /// If you want multiple [TextAutoSizing]s to have the same text size, give all
  /// of them the same [AutoSizeGroup] instance. All of them will have the
  /// size of the smallest [TextAutoSizing]
  final TextAutoSizingController? controller;
  ...
  /// use the [MediaQueryData] obtained from the ambient
  ...
  /// Creates a [TextAutoSizing] widget.
  ///
  /// If the [style] argument is null, the text will use the style from the
  /// closest enclosing [DefaultTextStyle].
  const TextAutoSizing(String this.data, {
    Key? key,
    this.textKey,
    this.style,
    this.strutStyle,
    ...
    this.maxLines,
    this.semanticsLabel,
  }) : textSpan = null, super(key: key);

  /// Creates a [TextAutoSizing] widget with a [TextSpan].
  const TextAutoSizing.rich(TextSpan this.textSpan, {
    Key? key,
    this.textKey,
    this.style,
    this.strutStyle,
    ...
    this.maxLines,
    this.semanticsLabel,
  }) : data = null, super(key: key);

  @override
  _TextAutoSizingState createState() => _TextAutoSizingState();
}

class _TextAutoSizingState extends State<TextAutoSizing> {
  @override
  void initState() {
    super.initState();
  ...
  @override
  void didUpdateWidget(TextAutoSizing oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (oldWidget.controller != widget.controller) {
      oldWidget.controller?._remove(this);
      widget.controller?._register(this);
    }
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, size) {
      ...
      if (style!.fontSize == null) {
        style = style.copyWith(fontSize: TextAutoSizing._defaultFontSize);
      }
      ...
      if (widget.controller != null) {
        ...
        text = _buildText(widget.controller!._fontSize, style, maxLines);
      } else {
  ...
  void _validateProperties(TextStyle style, int? maxLines) {
    ...
    if (widget.presetFontSizes == null) {
      assert(widget.stepGranularity >= 0.1, 'StepGranularity must be greater than or equal to 0.1. It is not a good idea to resize the font with a higher accuracy.');
  ...
  List _calculateFontSize(
    ...
    final scaler = widget.textScaleFactor != null
    ...
    if (presetFontSizes == null) {
      final double defaultFontSize = style!.fontSize!.clamp(widget.minFontSize, widget.maxFontSize).toDouble();
      final double defaultScale = scaler.scale(defaultFontSize) / style.fontSize!;
      if (_checkTextFits(span, defaultScale, maxLines, size)) {
        return <Object>[scaler.scale(defaultFontSize), true];
      }
    ...
    while (left <= right) {
      ...
      double scale;
      if (presetFontSizes == null) {
        scale = scaler.scale(mid * widget.stepGranularity) / style!.fontSize!;
      } else {
        scale = scaler.scale(presetFontSizes[mid]) / style!.fontSize!;
      }
    ...
    double fontSize;
    if (presetFontSizes == null) {
      fontSize = scaler.scale(right * widget.stepGranularity);
    } else {
      fontSize = scaler.scale(presetFontSizes[right]);
    }
  ...
  Widget _buildText(double fontSize, TextStyle style, int? maxLines) {
    ...
    if (widget.data != null) {
      return Text(
        widget.data!,
        key: widget.textKey,
        style: style.copyWith(fontSize: scaler.scale(fontSize)),
        strutStyle: widget.strutStyle,
...
class TextAutoSizingController {
  final _listeners = <_TextAutoSizingState, double>{};
  var _widgetsNotified = false;
  var _fontSize = double.infinity;

  void _register(_TextAutoSizingState text) {
    _listeners[text] = double.infinity;
  }

  void _updateFontSize(_TextAutoSizingState text, double maxFontSize) {
  ...
  void _remove(_TextAutoSizingState text) {

...
class _SwiperState extends State<Swiper> with SingleTickerProviderStateMixin {
  ...
  initState() {
    ...
    _rightIconAnimation = _controller.drive(Tween<Double>(begin: 0.0, end: 0.0));

    _onSwipeLeft = widget.onLeftSwipe ?? (details) {
      console.log("Left Swipe Not Provided");
    };

    _onSwipeRight = widget.onRightSwipe ?? (details) {
      console.log("Right Swipe Not Provided");
    };
  ...
  void _runAnimation({required bool onRight, details}) {
    ...
    if (onRight) {
      _leftIconAnimation = Tween(begin: 0.0, end: 1.0).animate(CurvedAnimation(curve: Curves.decelerate, parent: _controller));
    } else {
      _rightIconAnimation = Tween(begin: 0.0, end: 1.0).animate(CurvedAnimation(curve: Curves.decelerate, parent: _controller));
    }
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onPanUpdate: (details) {
        if (details.delta.dx.isGreaterThan(1) && widget.onRightSwipe.isNotNull) {
          _runAnimation(onRight: true,  details: details);
        }
        if (details.delta.dx.isLessThan(-1) && widget.onLeftSwipe.isNotNull) {
          ...
          Row(
            ...
            children: <Widget>[
              AnimatedOpacity(
                ...
                child: widget.rightSwipeWidget ?? Icon(
              ...
              AnimatedOpacity(
                ...
                duration: widget.animationDuration,
                curve: Curves.decelerate,
                child: widget.leftSwipeWidget ?? Icon(

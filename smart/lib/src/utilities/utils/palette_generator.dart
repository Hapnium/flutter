...
class _FromByteDataArgs {
  const _FromByteDataArgs(this.encodedImage, {
...
class PaletteGenerator with Diagnosticable {
  ...
  PaletteGenerator.fromColors(this.paletteColors, {this.targets = const <PaletteTarget>[]}) : selectedSwatches = <PaletteTarget, PaletteColor>{} {
    _sortSwatches();
    _selectSwatches();
  }
  ...
  /// [compute].
  static Future<PaletteGenerator> fromByteData(EncodedImage encodedImage, {
    Rect? region,
    ...
    assert(region == null || (region.bottomRight.dx <= encodedImage.width && region.bottomRight.dy <= encodedImage.height), 'Region $region is outside the image ${encodedImage.width}x${encodedImage.height}');
    assert(encodedImage.byteData.lengthInBytes ~/ 4 == encodedImage.width * encodedImage.height, "Image byte data doesn't match the image size, or has invalid encoding. The encoding must be RGBA with 8 bits per channel.");

    // In order to use an isolate, make a recursive compute call. This is done
    // this way to avoid changing the rest of the function.
    if (useIsolate) {
      return compute((_FromByteDataArgs message) => fromByteData(message.encodedImage,
    ...
    return PaletteGenerator.fromColors(colors, targets: targets);
  }
  ...
  static Future<PaletteGenerator> fromImage(ui.Image image, {
    Rect? region,
    int maximumColorCount = _defaultCalculateNumberColors,
    List<PaletteFilter> filters = const <PaletteFilter>[
      avoidRedBlackWhitePaletteFilter
  ...
  /// If [useIsolate] is true, the underlying [PaletteGenerator.fromByteData]
  /// will be run in an isolate via [compute].
  static Future<PaletteGenerator> fromImageProvider(ImageProvider imageProvider, {
    Size? size,
    Rect? region,
    int maximumColorCount = _defaultCalculateNumberColors,
    List<PaletteFilter> filters = const <PaletteFilter>[
    ...
    final ImageStream stream = imageProvider.resolve(ImageConfiguration(size: size, devicePixelRatio: 1.0));
    ...
    if (timeout != Duration.zero) {
      loadFailureTimeout = Timer(timeout, () {
        stream.removeListener(listener);
        imageCompleter.completeError(TimeoutException('Timeout occurred trying to load from $imageProvider'));
      });
    }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(IterableProperty<PaletteColor>('paletteColors', paletteColors,
...
class PaletteColor with Diagnosticable {
  ...
  static double _calculateContrast(Color foreground, Color background) {
    assert(background.a.toInt() == 0xff, 'background can not be translucent: $background.');
    if (foreground.a.toInt() < 0xff) {
  ...
  static int? _calculateMinimumAlpha(Color foreground, Color background, double minContrastRatio) {
    assert(background.a.toInt() == 0xff, 'The background cannot be translucent: $background.');
  ...
  static int _binaryAlphaSearch(Color foreground, Color background, double minContrastRatio, _ContrastCalculator calculator) {
    assert(background.a.toInt() == 0xff, 'The background cannot be translucent: $background.');
...
class _ColorVolumeBox {
  ...
  void _fitMinimumBox() {
    ...
    for (int i = _lowerIndex; i <= _upperIndex; i++) {
      ...
      if (color.r.toInt() > maxRed) {
        maxRed = color.r.toInt();
      }
      if (color.r.toInt() < minRed) {
        minRed = color.r.toInt();
      }
      if (color.g.toInt() > maxGreen) {
        maxGreen = color.g.toInt();
      }
      if (color.g.toInt() < minGreen) {
        minGreen = color.g.toInt();
      }
      if (color.b.toInt() > maxBlue) {
        maxBlue = color.b.toInt();
      }
      if (color.b.toInt() < minBlue) {
        minBlue = color.b.toInt();
      }
    }
    _minRed = minRed;
    _maxRed = maxRed;
    ...
    int compareColors(Color a, Color b) {
      ...
      switch (longestDimension) {
        case _ColorComponent.red:
          final int aValue = makeValue(a.r.toInt(), a.g.toInt(), a.b.toInt());
          final int bValue = makeValue(b.r.toInt(), b.g.toInt(), b.b.toInt());
          return aValue.compareTo(bValue);
        case _ColorComponent.green:
          final int aValue = makeValue(a.g.toInt(), a.r.toInt(), a.b.toInt());
          final int bValue = makeValue(b.g.toInt(), b.r.toInt(), b.b.toInt());
          return aValue.compareTo(bValue);
        case _ColorComponent.blue:
          final int aValue = makeValue(a.b.toInt(), a.g.toInt(), a.r.toInt());
          final int bValue = makeValue(b.b.toInt(), b.g.toInt(), b.r.toInt());
          return aValue.compareTo(bValue);
      }
    }
  ...
  PaletteColor getAverageColor() {
    ...
    for (int i = _lowerIndex; i <= _upperIndex; i++) {
      ...
      redSum += colorPopulation * color.r.toInt();
      greenSum += colorPopulation * color.g.toInt();
      blueSum += colorPopulation * color.b.toInt();
    }
...
class _ColorHistogram {
  ...
  _ColorCount? operator [](Color color) {
    final Map<int, Map<int, _ColorCount>>? redMap = _hist[color.r.toInt()];
    if (redMap == null) {
      return null;
    }
    final Map<int, _ColorCount>? blueMap = redMap[color.b.toInt()];
    if (blueMap == null) {
      return null;
    }
    return blueMap[color.g.toInt()];
  }

  void operator []=(Color key, _ColorCount value) {
    final int red = key.r.toInt();
    final int blue = key.b.toInt();
    final int green = key.g.toInt();
  ...
  void removeWhere(bool Function(Color key) predicate) {
    for (final Color key in _keys) {
      if (predicate(key)) {
        _hist[key.r.toInt()]?[key.b.toInt()]?.remove(key.g.toInt());
      }
    }
...
class _ColorCutQuantizer {
  _ColorCutQuantizer(this.encodedImage, {
  ...
  List<PaletteColor> _quantizeColors() {
    ...
    Color quantizeColor(Color color) {
      return Color.fromARGB(
        color.a.toInt(),
        color.r.toInt() & quantizeWordMask,
        color.g.toInt() & quantizeWordMask,
        color.b.toInt() & quantizeWordMask,
      );
    }
  ...
  List<PaletteColor> _quantizePixels(int maxColors, _ColorHistogram histogram) {

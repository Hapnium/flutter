...
class InAppNotification<T> implements InAppNotificationService<T> {
  /// Prefix for log messages.
  final String prefix = "Notify InApp Core";
  ...
  /// Creates an [InAppNotification] instance.
  InAppNotification({
    this.titleSize = 16,
  ...
  ToastificationItem _show({
    ...
    switch (type) {
      case ToastificationType.error:
        state = InAppState.error;
        break;

      case ToastificationType.info:
        state = InAppState.info;
        break;

        case ToastificationType.warning:
        state = InAppState.warning;
        break;

      default:
        state = InAppState.success;
        break;
    }
  ...
  void custom({
    ...
    if(showLogs) {
      console.debug("Building `custom` notification", prefix: prefix);
    }
    ...
    if(showLogs) {
      console.info("Notification built successfully", prefix: prefix);
    }
  }
  ...
  void error({
    ...
    if(showLogs) {
      console.debug("Building `error` notification", prefix: prefix);
    }
  ...
  void info({
    ...
    if(showLogs) {
      console.debug("Building `info` notification", prefix: prefix);
    }
  ...
  void tip({required String message, int duration = 5, Color? color, Color? textColor}) async {
    if(showLogs) {
      console.debug("Building `tip` notification", prefix: prefix);
    }
  ...
  void success({
    ...
    if(showLogs) {
      console.debug("Building `success` notification", prefix: prefix);
    }
  ...
  void warn({
    ...
    if(showLogs) {
      console.debug("Building `warn` notification", prefix: prefix);
    }
